 ...
 *
 * Engine per track:
 *   - track.audio     → <audio> element (our files, full control)
 *   - track.spotifyId → Spotify embed via the iFrame API; the embed stays visible
 *                       (Spotify's terms) and doubles as the progress/seek UI.
 */
...
export default function PlayerDock() {
  ...
  // Give the page room for the dock (footer links stay reachable).
  useEffect(() => {
    document.body.classList.toggle('has-dock', open);
  }, [open]);

  // ---- Spotify engine bootstrap (script loaded once, on first Spotify track)
  const ensureSpotify = () => {
      ...
      window.onSpotifyIframeApiReady = (api) => {
        ...
        api.createController(mount, { width: '100%', height: 80, uri: 'spotify:track:' + (currentTrack.get()?.spotifyId ?? '') }, (c) => {
          ...
          c.addListener('playback_update', (e) => {
            ...
            status.set({ ...cur, engine: 'spotify', playing: !d.isPaused, buffering: d.isBuffering, positionMs: d.position, durationMs: d.duration });
  ...
  const seek = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (track?.audio && audio.current) audio.current.currentTime = ms / 1000;
    else if (spotify.current) spotify.current.seek(ms / 1000);
  };
  ...
  return (
    ...
    >
      ...
      {/* progress hairline across the top (audio engine) */}
      {!spotifyEngine && (
      ...
      <div className="wrap grid grid-cols-[auto_1fr_auto] items-center gap-3 py-2.5 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:gap-6">
        {/* now playing */}
        ...
        {/* right: seek + time (audio) / links + close */}
        <div className="flex items-center justify-end gap-2">
          {!spotifyEngine && (
            <div className="hidden items-center gap-3 md:flex">
              ...
              <input
                type="range"
                className="seek w-40 accent-sky-400"
                min={0}
          ...
          <ul className="hidden items-center gap-0.5 text-buff-400 lg:flex" aria-label="Ouvir noutra plataforma">
            {track?.links?.spotify && <li><a className="icon-btn h-9 w-9" href={track.links.spotify} target="_blank" rel="noopener" aria-label="Abrir no Spotify"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d={siSpotify.path} /></svg></a></li>}
            {track?.links?.youtube && <li><a className="icon-btn h-9 w-9" href={track.links.youtube} target="_blank" rel="noopener" aria-label="Abrir no YouTube"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d={siYoutube.path} /></svg></a></li>}
            {track?.links?.apple && <li><a className="icon-btn h-9 w-9" href={track.links.apple} target="_blank" rel="noopener" aria-label="Abrir na Apple Music"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d={siApplemusic.path} /></svg></a></li>}
          </ul>
          <button className="icon-btn" type="button" onClick={close} aria-label="Fechar leitor"><X size={20} /></button>
        </div>
      </div>

      {/* Spotify embed — visible per Spotify's terms; hidden only while our own audio plays */}
      <div className={`wrap pb-2 ${spotifyEngine ? '' : 'hidden'}`}>
        <div ref={spotifyHost} className="spotify-host h-20 overflow-hidden rounded-xl bg-ink-800" />
        {st.error && <p className="mt-1 text-xs text-buff-300">{st.error}</p>}
        <p className="mt-1 text-[0.7rem] text-cream-500">Faixa completa para quem tem sessão iniciada no Spotify; caso contrário, pré-visualização de 30 s.</p>
      </div>
    </div>
  );
}
